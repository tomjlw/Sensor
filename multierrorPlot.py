...
def error(data_drone1, data_drone2, data_drone3):
    ...
    for i in range(0, maxRun):
        ...
        for i_batch in range(0, numberBatch):
            predictedPosition5 = RSSI_Loc_BatchedGD_5par(data_drone1[0:BatchSize_min], numEpoch,
                                                         numIterations, threshold, learning_rate)[0]
            error = haversine(predictedPosition5[0], predictedPosition5[1],cheater_lat, cheater_lon)

            error_1drones.append(error)
        ###END 1 DRONE
    ...
    # #     # move origin
            ...
            for i in range(0, len(centers)):
                [result_latlonalt[i][0], result_latlonalt[i][1]]  =  utm.to_latlon(centers[i][0][0]+x_org,
                                                                                   centers[i][0][1]+y_org, zone_number, zone_letter)
                errorsBlackBox_2d.append(getDistance(result_latlonalt[i][0], result_latlonalt[i][1],
                                                     cheater_alt, cheater_lat, cheater_lon, cheater_alt)[0])

            error_2drones.append(sum(errorsBlackBox_2d)/len(errorsBlackBox_2d))
            ###END 2 DRONES
        ...
        #####END 3 DRONES
    derror.append(stddev(error_1drones))
    derror.append(stddev(error_2drones))
    derror.append(stddev(error_3drones))
    return derror
...
for num in samples:
    ...
    plt.bar(xaxis, real_error, align='center', alpha=0.5, color = 'blue')
    plt.xticks(xaxis, tuple(drones))
    plt.ylabel("Localization error/m", fontsize=20)
    plt.xlabel("Number of Drones", fontsize=20)
    plt.title('Localization Error for {a} drones for {b} Samples'.format(a=len(drones), b = samples[count-1]))
    count += 1
...
#
plt.tight_layout()
plt.savefig("Drone & Error.png", dpi=600)
